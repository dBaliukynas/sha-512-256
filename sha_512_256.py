...
class Sha512_256:
	...
	def print_result(self, digested_message):
		"""Atspausdina paduotą maišos reikšmę komandinėje eilutėje.
	...
	def write_to_file(self, digested_message):
		...
		try:
			...
			self.print_error_message(f"Nėra suteiktos privilegijos rašyti į failą {self.options.output_filename}.")
			raise SystemExit(1)
	...
	def read_from_file(self):
		...
		try:
			...
			self.print_error_message(f"Failas {self.options.input_filename} yra katalogas.")
			raise SystemExit(1)
		except PermissionError:
			self.print_error_message(f"Nėra suteiktos privilegijos skaityti failą {self.options.input_filename}.")
			raise SystemExit(1)
